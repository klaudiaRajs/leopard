...
class FileAnalyzer extends CI_Controller{
    ...
    function analyzeUpload($fileName)
    {
        ...
        $formattedContents = TokenPresenter::getFormattedContents($tokenizer->getAll());
        $this->load->view('codePresenter', ['fileContents' => $formattedContents]);
    }

    private function getColorCoded($fileContents)
    {
        $unifiedContents = str_replace("\r", '', $fileContents);
        $lines = explode("\n", $unifiedContents);

        foreach ($lines as $lineNumber => $lineContent) {
            $count = substr_count($lineContent, "    ");
            $number = str_pad($lineNumber + 1, 2, ' ', STR_PAD_LEFT);
            $displayableLineNumber = '<span style="color:#a6a6a6;">' . $number . '</span>';
            echo $displayableLineNumber . "&nbsp;&nbsp;";
            if ($count != 0) {
                for ($i = 0; $i < $count; $i++) {
                    echo "&nbsp;&nbsp;&nbsp;&nbsp;";
                }
            ...
            $lineContent = $this->getColorCodedLine($lineContent);

            if ($number == 1 || $number == 2) {
                echo htmlentities($lineContent) . "<br>";
            } else {
                echo $lineContent . "<br>";
            }
        }
    ...
    public function getColorCodedLine($lineContent)
    {
        $lineContent = RegularExpressionsAnalyzer::findGlobals($lineContent);
        $lineContent = RegularExpressionsAnalyzer::findUsingStaticMethods($lineContent);
        $lineContent = RegularExpressionsAnalyzer::findDeprecatedFunctions($lineContent);
        $lineContent = SyntaxColourHelper::colourSyntax($lineContent);
        return $lineContent;
    }
}
...
class tokenizer{
    ...
    private function parse($fileContents)
    {
        ...
        foreach ($tokens as $token) {
            if (is_array($token)) {
                $this->tokens[] = new token($token[0], $token[1], $token[2], token_name($token[0]));
                $lastLine = $token[2];
                ...
                $this->tokens[] = new token($additional[$token], $token, $lastLine, $additional[$token]);
            } else {
                print_r($token);
                exit;
            }
...
class token{
    ...
    public function __construct($tokenIdx, $content, $lineNumber, $tokenName)
    {
...
class TokenPresenter{
        ...
        foreach ($tokens as $token) {
            if ($token->tokenIdx == T_WHITESPACE && preg_match('/[\n]+/', $token->content)) {
                ...
                foreach($lines as $index => $line) {
                    ...
                    if (empty($line)) {
                        ...
                        $tokensForLine[] = new TokenView(T_WHITESPACE, $line, token_name(T_WHITESPACE));
                    }
                }
                continue;
            }
            ...
            if (strpos($token->content, "\n") !== false) {
                ...
                foreach($lines as $lineNumber => $lineContent) {
                    ...
                    $tokensForLine[] = new TokenView($token->tokenIdx, $lineContent, $token->tokenName);
    ...
    private static function getFormattedToken(TokenView $token)
    {
        ...
        return sprintf('<span style="%s;" title="%s">%s</span>', implode(';', $style), $token->tokenName, $text);
    }
}
...
class TokenView {
    ...
    public function __construct($tokenIdx, $content, $tokenName)
    {
        $this->tokenIdx = $tokenIdx;
        $this->content = $content;
        $this->tokenName = $tokenName;
    ...
    {
        return new TokenView($token->tokenIdx, $token->content, $token->tokenName);
    }
}